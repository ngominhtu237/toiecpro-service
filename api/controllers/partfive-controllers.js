...
exports.create = (req, res, next) => {
    ...
    for(let i=0; i<40; i++) {
        ...
        tempQues.answer = "D";
        tempQues.idPartFive = _id;
        mQuestionPartFive.push(tempQues);
    }
    ...
    PartFive.create(part)
    .then(content => {
        console.log("create partfive successfully! (not include question)");
        QuestionPartFive.bulkCreate(mQuestionPartFive,{ individualHooks: true })
            .then(ques => {
                res.status(200).json({
                    message: 'Create partfive successfully!'
                })
            })
            .catch(err => {
