...
exports.create = (req, res, next) => {
    ...
    for(let i=0; i<30; i++) {
        ...
        tempQues.answer = "D";
        tempQues.idPartFour = _id;
        mQuestionPartFour.push(tempQues);
    }
    ...
    PartFour.create(part)
    .then(content => {
        console.log("create partfour successfully! (not include question)");
        QuestionPartFour.bulkCreate(mQuestionPartFour,{ individualHooks: true })
            .then(ques => {
                res.status(200).json({
                    message: 'Create partfour successfully!'
                })
            })
            .catch(err => {
